				...
				if(rsPass){*/
					...
					.then(async function (rsAccRoles){
						if(rsAccRoles.length>0 && await generals.thisRole([{"accountId":rsUser['rows'][0].id},{"roleId":[{"id":5},{"id":7}]}]) ){
							...
							var token =  await servToken.newToken(dataAccount,allRole,dataShop,dataPeople,'login',today) //generar Token 									
							res.status(200).json({data:{"result":true,"message":"Usted a iniciado sesión " + rsUser['rows'][0].email ,"token":token,tokenRole,"account":dataAccount,"role":allRole,"shop":dataShop,"dateTime":today}});							
